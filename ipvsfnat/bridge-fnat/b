...
ip netns exec ${ns} ipvsadm -A -u 192.168.20.3:80 -s ${rr}
ip netns exec ${ns} ipvsadm -a -u 192.168.20.3:80 -r 192.168.10.5:8080 -m
...
if [ "$1" = "yes" ]; then
	...
	ip netns exec ${ns} ipvsadm -a -t 192.168.20.3:80 -r 192.168.10.3:80 -g     
	#ip netns exec ${ns} ipvsadm -A -t 192.168.10.3:80 -s ${rr}
	#ip netns exec ${ns} ipvsadm -a -t 192.168.10.3:80 -r 192.168.10.5:8080 -g     
	#ip netns exec ${ns} ipvsadm -a -t 192.168.10.3:80 -r 192.168.10.6:8080 -g     
elif [ "$1" = "no" ]; then 
	ip netns exec ${ns} ipvsadm -d -t 192.168.20.3:80 -r 192.168.10.5:8080 -g     
	ip netns exec ${ns} ipvsadm -D -t 192.168.20.3:80
	ip netns exec ${ns} ipvsadm -d -t 192.168.10.3:80 -r 192.168.10.5:8080 -g     
